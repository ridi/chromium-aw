// CrossOriginOpenerPolicy.java is auto generated by mojom_bindings_generator.py, do not edit


// Copyright 2014 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// This file is autogenerated by:
//     mojo/public/tools/bindings/mojom_bindings_generator.py
// For:
//     services/network/public/mojom/cross_origin_opener_policy.mojom
//

package org.chromium.network.mojom;

public final class CrossOriginOpenerPolicy {
    private static final boolean IS_EXTENSIBLE = false;

    public static final int SAME_ORIGIN = 0;
    public static final int SAME_ORIGIN_ALLOW_POPUPS = 1; // SAME_ORIGIN + 1
    public static final int UNSAFE_NONE = 2; // SAME_ORIGIN_ALLOW_POPUPS + 1

    public static boolean isKnownValue(int value) {
        return value >= 0 && value <= 2;
    }

    public static void validate(int value) {
        if (IS_EXTENSIBLE || isKnownValue(value)) return;
        throw new org.chromium.mojo.bindings.DeserializationException("Invalid enum value.");
    }

    private CrossOriginOpenerPolicy() {}
}